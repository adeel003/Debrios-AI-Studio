...
import { UserSquare2, Phone, Award, Plus, AlertCircle, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
function ComplianceBadge({ label, date }: { label: string; date: string | null }) {
  ...
  const nearExpiry = isPast(addDays(expiryDate, -30)); // 30 days warning
...
export function OperativeDirectory() {
  ...
  const fetchDrivers = async () => {
    ...
    try {
      setLoading(true);
      const { data, count } = await driverService.getTenantDrivers(profile.tenant_id, {
        page,
        pageSize
      });
      setDrivers(data);
      ...
      handleError(err, 'Drivers:fetchDrivers');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Manage your fleet drivers and their availability.</p>
        </div>
      ...
      {drivers.length === 0 ? (
        <div className="bg-white p-16 text-center rounded-xl border border-gray-200 shadow-sm">
          <div className="flex flex-col items-center max-w-sm mx-auto">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1">No drivers yet</h3>
            <p className="text-sm text-gray-500 mb-6">
              Add your first driver to start assigning loads and tracking fleet availability.
            </p>
            <button 
              onClick={() => handleOpenModal()}
        ...
        <>
            ...
            {drivers.map((driver) => (
              <div key={driver.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="mt-6 space-y-3">
                  ...
                  <div className="flex items-center text-sm text-gray-600">
                    <span className="font-medium mr-2">Iqama:</span>
                ...
                <div className="mt-6 pt-6 border-t border-gray-100 flex justify-end space-x-2">
                  <button 
          ...
          {/* Pagination */}
          <div className="px-6 py-4 bg-white border border-gray-200 rounded-xl mt-6 flex items-center justify-between shadow-sm">
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">
                {editingDriver ? `Edit Driver: ${editingDriver.full_name}` : 'Add New Driver'}
              </h2>
